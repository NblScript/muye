...
**动作**：
1. 端口占用预检（防止上一场演示的残留进程造成“假就绪”）
2. 启动主 API（端口 18000）
3. 启动前端（端口 5173）
4. 按剧本播种演示数据：巡检 → 检测 → 决策；阶段 4 喷洒由前端「确认起飞」触发

演示入口只运行 `api_app` 视图层，不启动主处理链与 YOLO API（端口 8010），虫情来自确定性种子数据。

**参数**：
- `--mode virtual|px4`（默认 virtual；px4 额外尽力启动 PX4 SITL + Gazebo 供现场观感）
- `--takeoff manual|auto`（默认 manual）
- `--api-port 18000`
- `--frontend-port 5173`

**预设场景**：`./scripts/demo_scenario.sh <场景名>` 可预设环境变量和样例图片（demo.sh 尊重已导出的场景变量，不再无条件覆盖）：
...
**触发**：用户通过前端上传图片（生产链路）；演示模式由种子数据驱动

...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "从害虫图片批量构建历史决策案例并写入 ChromaDB decisions 集合。\n"
            "供 DecisionRouter 的历史案例评分与多智能体会诊经验检索使用。\n"
            "需要 config/api_keys.env 提供 QWEN_API_KEY（DashScope 向量化）。"
        )
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=20260501,
        help="Random seed for reproducible output (default 20260501)",
    )
    ...
    for i, img_path in enumerate(sample):
        ...
        # 确定性 request_id 与时间戳：同一 --seed 下可复现，便于验收与去重
        request_id = f"seed-history-{i:04d}"
        seeded_date = f"2026-05-{(i % 28) + 1:02d}"
        ...
        doc = build_historical_decision_document(
            ...
            field_id=field_id,
            crop_name=crop_name,
            timestamp=f"{seeded_date}T08:00:00",
        )
        if doc:
